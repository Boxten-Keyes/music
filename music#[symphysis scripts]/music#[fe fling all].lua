			...
			task.spawn(function()
				if state then
					if loopflinging == true then
						notify("fe fling all | symphysis", "loopfling is already enabled.", 2)
					else
						task.spawn(loopfling)
						notify("fe fling all | symphysis", "loopfling started.", 2)
						task.spawn(dc)
						task.spawn(af)
					...
					if loopflinging == false then
						notify("fe fling all | symphysis", "loopfling is already disabled.", 2)
					else
						task.spawn(unloopfling)
						notify("fe fling all | symphysis", "loopfling stopped.", 2)
						task.spawn(rc)
						task.spawn(uaf)
	...
	local function give()
		...
		t.Equipped:Connect(function()
			task.spawn(function()
				...
				notify("fe fling all | symphysis", "flinging everyone once.", 2)
			end)
		end)
	end
...
function init()
	task.spawn(function() notify("fe fling all | symphysis", "made by frwlsr. enjoy!", 4) end)
	task.spawn(function() givelooptool("start loopfling", true) task.wait() givelooptool("stop loopfling", false) task.wait() givetool() end)
end
